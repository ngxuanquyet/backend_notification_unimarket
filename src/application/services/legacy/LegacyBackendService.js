...
class LegacyBackendService {
  ...
  async processSePayWebhook({ headers, body }) {
    verifySePayWebhook({ headers });

    const transaction = normalizeSePayWebhook(body || {});
    ...
    await db.collection('paymentTransactions').doc(transactionDocId).set(transactionPayload, {
      merge: true
    });
  ...
  async sendChatNotification({ senderId, body }) {
    const { conversationId, text } = body || {};

    if (!conversationId || !text) {
      throw httpError(400, 'conversationId and text are required');
    }
    ...
    if (!participantIds.includes(senderId)) {
      throw httpError(403, 'Sender is not a participant of this conversation');
    }
  ...
  async confirmBuyNowPurchase({ decodedToken, body }) {
    const buyerId = decodedToken.uid;
    const purchase = normalizePurchaseRequest(body || {});
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      const imageUrls = Array.isArray(product.imageUrls)
        ? product.imageUrls.filter((item) => typeof item === 'string')
        : [];
      ...
      const paymentExpiresAt = isTransferPayment ? Date.now() + TRANSFER_PAYMENT_WINDOW_MS : null;
  ...
  async updateOrderStatus({ actorId, orderId, body }) {
    const statusUpdate = normalizeOrderStatusUpdateRequest(body || {});
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      validateOrderStatusTransition({ currentStatus, nextStatus, deliveryMethod });
  ...
  async checkTransferPayment({ buyerId, orderId }) {
    if (!orderId) {
      throw httpError(400, 'orderId is required');
    }
    ...
    const result = await confirmTransferPayment({ orderId, order, paymentMatch });
    await finalizeTransferPayment(result);
  ...
  async getBuyerOrders(userId) {
    await expireOverdueTransferOrdersForUser(userId);
  ...
  async setUserLock({ actorId, targetUid, disabled }) {
    if (!targetUid) {
  ...
  async deleteUser({ actorId, targetUid }) {
    if (!targetUid) {
      throw httpError(400, 'uid is required');
    }
    if (targetUid === actorId) {
  ...
  async requireAdminAuthFromHeader(authorizationHeader) {
    const decodedToken = await this.requireAuthFromHeader(authorizationHeader);
